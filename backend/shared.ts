...
export async function fetchTradegateQuote(isin: string): Promise<Quote> {
  ...
  const raw = await response.json() as { bid?: string | number; close?: string | number };
  ...
  return {
    isin,
    bid,
    change: bid !== null && close !== null && close > 0 ? ((bid / close) - 1) * 100 : null
  };
}
...
export async function fetchTradegateQuotes(isins: string[]): Promise<Record<string, Quote>> {
  const unique = [...new Set(isins.map((isin) => isin.trim().toUpperCase()).filter(Boolean))];
  const results = await Promise.all(unique.map((isin) => fetchTradegateQuote(isin)));
...
export function readIsinsFromUrl(url: URL): string[] {
  ...
  return query.split(',').map((isin) => isin.trim()).filter(Boolean);
}
